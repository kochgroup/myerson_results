assert False, "using outdated version of Myerson calculator"
# %%
import sys, os
...
model = TanhModel(model).to(device)
...
with open("3KCK_tanh_adversarial_myerson_values.pkl", "wb") as f:
    pickle.dump(explainer.my_values, f)
# %%
from explanations.visualizations import draw_colorbar, draw_with_weights
with open("3KCK_tanh_adversarial_myerson_values.pkl", "rb") as f:
    data = pickle.load(f)
...
# graph_adversarial2.to(device)
# model(graph_adversarial2.x, graph_adversarial2.edge_index, torch.zeros(graph_adversarial2.x.shape[0], dtype=int, device=device))
