...
for i in range(9, 51):
    ...
    for idx in tqdm(size_idxs[i][:10]):
        ...
        my_10_time= time.time() - start

        start = time.time()
        explainer = MyersonSamplingExplainer(dataset[idx], model,
                                             number_of_samples=100)
        my_100 = explainer.sample_all_myerson_values()
        my_100_time = time.time() - start

        start = time.time()
        explainer = MyersonSamplingExplainer(dataset[idx], model,
                                             number_of_samples=500)
        my_500 = explainer.sample_all_myerson_values()
        my_500_time = time.time() - start

        start = time.time()
        explainer = MyersonSamplingExplainer(dataset[idx], model,
                                             number_of_samples=1000)
        my_1000 = explainer.sample_all_myerson_values()
        my_1000_time = time.time() - start

        start = time.time()
        explainer = MyersonSamplingExplainer(dataset[idx], model,
                                             number_of_samples=5000)
        my_5000 = explainer.sample_all_myerson_values()
        my_5000_time = time.time() - start

        d = {'my_10': my_10,
             'my_10_time': my_10_time,

             'my_100': my_100,
             'my_100_time': my_100_time,
             
             'my_500': my_500,
             'my_500_time': my_500_time,

             'my_1000': my_1000,
             'my_1000_time': my_1000_time,

             'my_5000': my_5000,
             'my_5000_time': my_5000_time,

             'smiles': dataset[idx].smiles,
             'pred': explainer.calculate_prediction()}
        helper.update({idx: d})
    my_exact.update({i: helper})
    with open('data/mc_my.yaml', 'a') as f:
